...
class SARemixInstaller(ctk.CTk):
    ...
    def create_main_page(self):
        ...
        # Add progress bar and download info label
        self.progress_bar = ctk.CTkProgressBar(self.left_frame, width=360)
        self.progress_bar.grid(row=2, column=0, pady=(10, 5), padx=20)
        self.progress_bar.set(0)

        self.download_info_label = ctk.CTkLabel(self.left_frame, text="Download Speed: 0 MB/s", width=360)
        self.download_info_label.grid(row=3, column=0, pady=(0, 10), padx=20)

        self.right_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent", border_width=2, border_color="#FFCC70", width=380, height=500)
        self.right_frame.grid(row=0, column=1, sticky="nsew", padx=(10, 0))
        self.right_frame.grid_propagate(False)
        self.right_frame.grid_rowconfigure((0, 1, 2, 3, 4, 5, 6), weight=1)
        ...
        # New section for Install buttons
        self.install_frame = ctk.CTkFrame(self, fg_color="transparent", border_width=2, border_color="#FFCC70", height=80)  # Increased height
        self.install_frame.grid(row=3, column=0, sticky="ew", padx=20, pady=(10, 20))
        self.install_frame.grid_propagate(False)  # Prevent frame from shrinking
        self.install_frame.grid_columnconfigure((0, 1), weight=1)  # Equal weight for both columns
        
        self.install_remix_button = ctk.CTkButton(self.install_frame, text="Install Remix 0.5.4", 
                                                    command=self.Download_Remix_0_5_4, 
        ...
        self.install_rtx_config_button = ctk.CTkButton(self.install_frame, text="Install RTX Remix config", 
                                                  command=self.install_rtx_config, 
                                                  fg_color="transparent", border_width=1, 
                                                  border_color="#FFCC70", width=200, 
                                                  hover_color="#5c5c5a")
    ...
    def install_rtx_config(self):
        ...
        try:
            url = "https://github.com/KCDQYaniss/SA-Remix-necessary-mods/releases/download/aupx/rtx.conf"
            ...
            # Reset progress bar
            self.progress_bar.set(0)
            self.download_info_label.configure(text="Download Speed: 0 MB/s")
            
            # Download with progress tracking
            response = self.session.get(url, stream=True)
            response.raise_for_status()
            
            config_path = os.path.join(self.folder_selected, "rtx.conf")
            self.download_file(url, config_path, total_size)
            
            messagebox.showinfo("Success", "RTX Remix config file has been installed successfully.")
            logging.info("RTX Remix config file installed successfully")
            
        except Exception as e:
            error_message = f"Failed to download RTX Remix config: {str(e)}"
    ...
    def get_7zip_path(self):
        """Get path to 7zip, either bundled or system installed"""
        # First check bundled 7zip in the same directory as the executable
        ...
        bundled_7zip = os.path.join(application_path, "7zip", "7z.exe")
        
        if os.path.exists(bundled_7zip):
            return bundled_7zip
        ...
        def download_thread():
            ...
            if not seven_zip_path:
                self.after(0, lambda: messagebox.showerror(
                    "Error", 
                    "7-Zip not found. Please make sure 7-Zip is installed or the application includes 7zip folder."
                ))
                return

            self.after(0, lambda: self.update_download_status("Downloading SA Remix"))
            self.after(0, lambda: self.disable_button(self.update1))
            self.after(0, lambda: self.update1_status.configure(text="Downloading"))

            # Fetch the latest release and extract the URLs for the split ZIP files
            owner = "KCDQYaniss"
            repo = "GTASA-Remix"
            response = self.session.get(f"https://api.github.com/repos/{owner}/{repo}/releases/latest")
            response.raise_for_status()
            release_data = response.json()
            base_url = release_data['assets'][0]['browser_download_url'].rsplit('/', 1)[0] + '/'
            zip_files = [{"url": f"{base_url}{asset['name']}", "filename": asset['name']} for asset in release_data['assets']]

            try:
                # Download all parts
                for zip_part in zip_files:
                    self.after(0, lambda: self.update_download_status(f"Downloading {zip_part['filename']}"))
                    file_path = os.path.join(self.folder_selected, zip_part["filename"])
                    
                    # Get file size first
                    response = self.session.head(zip_part["url"])
                    total_size = int(response.headers.get('content-length', 0))
                    
                    # Download the file
                    self.download_file(zip_part["url"], file_path, total_size)

                # Verify all required files exist before extraction
                all_files_present = all(
                    os.path.exists(os.path.join(self.folder_selected, zip_file["filename"]))
                    for zip_file in zip_files
                )
                
                if not all_files_present:
                    raise Exception("Some required files are missing before extraction")

                try:
                    self.after(0, lambda: self.update_download_status("Extracting files..."))
                    main_zip = os.path.join(self.folder_selected, "SARemix.zip")

                    # Use 7-Zip to extract
                    process = subprocess.run(
                        [seven_zip_path, 'x', main_zip, f'-o{self.folder_selected}', '-y'],
                        capture_output=True,
                        text=True,
                        check=True
                    )
                    ...
                    extracted_items = [f for f in os.listdir(self.folder_selected) 
                                    if f not in [zf["filename"] for zf in zip_files]]
                    
                    if not extracted_items:
                        raise Exception("No files were extracted")

                    # Clean up only after successful extraction
                    self.after(0, lambda: self.update_download_status("Cleaning up..."))
                    for zip_part in zip_files:
                        try:
                            cleanup_path = os.path.join(self.folder_selected, zip_part["filename"])
                            if os.path.exists(cleanup_path):
                                os.remove(cleanup_path)
                        except OSError as e:
                            logging.warning(f"Failed to remove file {zip_part['filename']}: {e}")
                    ...
                    error_msg = f"7-Zip extraction failed: {e.stderr}"
                ...
                self.after(0, lambda: self.update_download_status("SA Remix installation complete"))
    ...
    def Download_Remix_0_5_4(self, event=None):
        def download_thread():
            self.after(0, lambda: self.update_download_status("Downloading Remix 0.5.4"))
            if not self.folder_selected:
                messagebox.showwarning("Warning", "Please select a directory first.")
                return

            self.disable_button(self.install_remix_button)
            self.after(0, lambda: self.update1_status.configure(text="Updating"))
            try:
                download_url = "https://github.com/NVIDIAGameWorks/rtx-remix/releases/download/remix-0.5.4/remix-0.5.4-release.zip"
                file_name = "remix-0.5.4-release.zip"
                file_path = os.path.join(self.folder_selected, file_name)
                ...
                self.after(0, lambda: messagebox.showinfo("Success", "Remix 0.5.4 downloaded and extracted successfully."))
                logging.info("Remix 0.5.4 downloaded and extracted successfully.")
            except Exception as e:
                error_message = f"An error occurred: {str(e)}"
                self.after(0, lambda: messagebox.showerror("Error", error_message))
                logging.error(error_message)
            finally:
                self.after(0, lambda: self.update_download_status("Remix 0.5.4 download complete"))
                self.after(0, lambda: self.enable_button(self.install_remix_button))
                self.after(0, lambda: self.update1_status.configure(text="Update complete"))
                logging.info("Update of Remix 0.5.4 completed")
                if event:
                    event.set()
    ...
    def Download_all(self):
        ...
        def download_all_thread():
            try:
                ...
                # Download latest RTX Remix release
