...
class SARemixInstaller(ctk.CTk):
    ...
    def Download_SARemix(self, event=None):
        def download_thread():
            ...
            # URLs for the three RAR files
            rar_files = [
                {
                    "url": "https://github.com/KCDQYaniss/GTASA-Remix/releases/download/test/GTASA-Remix.part1.rar",
                    "filename": "part1.rar"
                },
                {
                    "url": "https://github.com/KCDQYaniss/GTASA-Remix/releases/download/test/GTASA-Remix.part2.rar",
                    "filename": "part2.rar"
                },
                {
                    "url": "https://github.com/KCDQYaniss/GTASA-Remix/releases/download/test/GTASA-Remix.part3.rar",
                    "filename": "part3.rar"
                }
            ]
            ...
            try:
                for rar_file in rar_files:
                    file_path = os.path.join(self.folder_selected, rar_file["filename"])
                    
                    # Get file size first
                    response = self.session.head(rar_file["url"])
                    total_size = int(response.headers.get('content-length', 0))
                    
                    # Download the file
                    self.download_file(rar_file["url"], file_path, total_size)

                # Extract the RAR files
                first_rar = os.path.join(self.folder_selected, "part1.rar")
                patoolib.extract_archive(first_rar, outdir=self.folder_selected)

                # Clean up RAR files after successful extraction
                for rar_file in rar_files:
                    try:
                        os.remove(os.path.join(self.folder_selected, rar_file["filename"]))
                    except OSError as e:
                        logging.warning(f"Failed to remove RAR file: {e}")
    ...
    def Download_all(self):
        ...
        def download_all_thread():
            try:
                ...
                # rtx_remix_event = Event()  # Commented out since we are not using it
                ...
                # Commenting out the RTX Remix download
                # self.Download_RTX_Remix("NVIDIAGameWorks", "rtx-remix", rtx_remix_event)
                # rtx_remix_event.wait()

                # New functionality to download and install Remix 0.5.4
                remix_event = Event()
                self.Download_Remix_0_5_4(remix_event)
                remix_event.wait()
                ...
                self.after(0, lambda: self.update1_status.configure(text="Update complete"))
                self.after(0, lambda: self.update2_status.configure(text="Update complete"))
                # self.after(0, lambda: self.update_rtx_remix_status.configure(text="Update complete"))  # Commented out
